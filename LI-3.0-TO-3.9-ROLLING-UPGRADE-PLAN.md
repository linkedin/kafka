...
Expect controller request count, allocation, network traffic, and convergence time to rise. The first full `UpdateMetadata` can be very large. Combined control was introduced partly to avoid building and holding large per-broker requests. Test controller startup and failover with production-scale metadata before proceeding.
...
- validate every LI feature carried into 3.9;
...
This section records what has been implemented and tested. It is not a declaration of production readiness.
...
- The 3.0 bridge compiles main and test sources with Scala 2.12 on the branch's Java 11 build toolchain. Its release archive completes the mixed-binary process test while the brokers run on Java 17.
...
- Bidirectional, non-empty byte fixtures now cover the common controller requests, recommended elections, metadata partition exclusion, and all supported LinkedIn private request APIs. Both artifacts decode the opposite artifact's fixtures and require byte-for-byte re-encoding.
- Matching constant tests pin private API IDs, recommended election value, LI local-offset timestamp, error 1107 mapping, and preferred-controller error 2000 on both branches. The 3.9 schema-scope test also requires all LinkedIn private APIs to be advertised only by ZooKeeper brokers, never KRaft brokers or controllers.
...
| ZooKeeper pagination | disabled-by-default setting carried into 3.9 |
...
| maintenance assignment exclusion | implemented in broker and direct ZooKeeper admin paths |
| observer, KafkaActions, topic policy | wrapper-compatible interfaces and hooks implemented |
...
| unofficial-client warning log | not forwarded: disabled in rendered repository configuration; observer classification remains |
| remote storage | use the Apache 3.9 implementation; repository configuration is disabled, pending live confirmation |

### Still required before completion

- Port direct 3.0 coverage for preferred-controller election/fallback and controlled-shutdown safety. The current mixed-process test calls API 1000 with controlled shutdown disabled, so it does not prove the full shutdown policy.
- Add focused tests for paginated ZooKeeper topic reads, a non-identity rack mapper, `KafkaActions` lifecycle callbacks, and the new KafkaAdminClient request/result paths. These paths compile, and some run indirectly through the wrapper. Current tests do not isolate their behavior.
