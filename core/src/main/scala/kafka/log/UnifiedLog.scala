...
class UnifiedLog(@volatile var logStartOffset: Long,
  ...
  private def maybeRoll(messagesSize: Int, appendInfo: LogAppendInfo): LogSegment = lock synchronized {
    ...
    if (segment.shouldRoll(new RollParams(config.maxSegmentMs, config.liMinSegmentMs, config.segmentSize,
      appendInfo.maxTimestamp, appendInfo.lastOffset, messagesSize, now))) {
