  ...
  def testFromPropsInvalid() {
    ...
    KafkaConfig.configNames().foreach(name => {
      name match {
        ...
        case KafkaConfig.OffsetsTopicMaxMessageBytesProp => assertPropertyInvalid(getBaseProperties(), name, "not_a_number", "-1")
        case KafkaConfig.OffsetsTopicMinInSyncReplicasProp => assertPropertyInvalid(getBaseProperties(), name, "not_a_number", "0")
        case KafkaConfig.OffsetsTopicMinCompactionLagMsProp => assertPropertyInvalid(getBaseProperties(), name, "not_a_number", "-1")
