...
class ListOffsetsRequestInstrumentationTest {
  ...
  @Test
  def testAbsentTimestampModesDoNotRecordZeroSizedSamples(): Unit = {
    val instrumentation = new ListOffsetsRequestInstrumentation(enabled = true)
    ...
    try {
      ...
      val histograms = KafkaYammerMetrics.defaultRegistry.allMetrics.asScala.collect {
        case (name, histogram: Histogram)
          if name.getMBeanName.contains("type=ListOffsetsRequestInstrumentation") =>
          name.getMBeanName -> histogram.count
      }
      assertEquals(1L, histograms.collectFirst {
        case (name, count) if name.contains("listBy=BY_TIMESTAMP") => count
      }.get)
      assertTrue(histograms.forall { case (name, count) =>
        name.contains("listBy=BY_TIMESTAMP") || count == 0L
      })
    } finally {
      instrumentation.close()
    }
