...
- Current 3.9 implementation and qualification: PR 600, `3.9-li-bridge/admission-dispositions`, including F25, F27 and mandatory scenario-6 checks.
- Complete local qualification: runtime source `f919812ba4255f07412aefcfb8d59246174dc266`. Later changes are documentation, tests, test selection and live-admission validation, not Kafka runtime code. The new live-admission checks have separate CLI regression evidence.
...
|---|---|---|
| Review the named plan and every open public PR | `LI-3.0-TO-3.9-ROLLING-UPGRADE-PLAN.md`; canonical runbook; GitHub inventory | 50 open PRs are listed in both tables. Recheck the exact PR set after any further publication. |
| Explain scope and dependencies | PR responsibility table, heads/bases, stack membership | Separate protocol, handlers, storage metrics, runner, auditor, verifier and release-gate layers. GitHub stack 582 has 39 upgrade PRs; stack 581 has nine. New members were appended and read back. CI 558/559 remain on independent release histories. No release branch was merged. |
| Apply the requested label | GitHub labels | All 48 upgrade PRs have `kafka-upgrade-august-2026`; CI 558/559 do not. |
...
| Address every review comment with evidence | Comment ledger, source/test decisions, GraphQL readback | All original 62 replies verified across 50 PRs; no new review threads. Later issue findings have published fixes and explicit qualification limits. Re-fetch after final publication. |
...
- `/tmp/li-review-readback-result.json`: 50 PRs, 62 original threads, no missing/mismatched replies and no new threads at that readback.
...
Failed runs remain failed. A source, archive or scenario change must be checked against the fingerprint before reuse. The post-f919 documentation/test/live-admission delta is checked separately; it is not a claim that commit identities are identical. Kafka runtime code and the client profile are unchanged.
