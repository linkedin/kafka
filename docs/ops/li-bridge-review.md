...
Failed runs remain failed. A source, archive or scenario change must be checked against the fingerprint before reuse. The post-f919 documentation/test/live-admission/fixture-orchestration delta is checked separately; it is not a claim that commit identities are identical. Kafka runtime code and the client profile are unchanged. F32 requires a complete run of the corrected process.

`/tmp/li-ci-600-small-evidence` retains the failed CLI refusal and concurrent churn assignment. `/tmp/li-reassignment-coexistence-before.log` records both call-site failures; `/tmp/li-reassignment-coexistence-confirmed` verifies the scoped repair and retains the unrelated pending assignment. `/tmp/li-final-coexisting-process` is the running complete process on immutable f919812ba4 / 8086d17968 archives. Its result must be read before closing this finding.
