...
**Do not deploy without approved published-artifact qualification and the release approvals.** A complete scenario-6 full-verifier run now passes on f919812ba4 / 8086d17968 / wrapper 1764cc95, followed by an independent strict clean/full/archive audit. The gate-test and admission follow-ups add separately verified checks without changing Kafka runtime code or the client profile. A later CI run exposed F32's fixture/metadata-churn collision. The repaired runner now passes both its deterministic coexistence probe and a complete updated process run with an independent archive/source audit. F28's inherited client-bootstrap limitation is an explicit client-floor release gate; this review does not choose a deployed client setting. The inventory covers 51 open PRs. Earlier passing or failed bundles retain their original source and coverage limits.
...
- Current 3.9 implementation and qualification: PR 601, `3.9-li-bridge/reassignment-coexistence`, including F25, F27 and mandatory scenario-6 checks.
- Complete local qualification: runtime source `f919812ba4255f07412aefcfb8d59246174dc266`. Later changes are documentation, tests, test selection, live-admission validation and fixture orchestration, not Kafka runtime code. New checks have separate evidence; the complete F32 process passes on runner a93bed480e with the same immutable runtime archives.
...
The new regression `TopicDeletionManagerTest.testBridgeDeletionWaitsForOfflineReplicaAfterControllerFailover` failed before the fix: ZooKeeper deletion ran without a metadata update. The bridge now uses the older Kafka behavior: send the metadata deletion update, retain offline replicas for retry when they return, and wait for their deletion acknowledgements. The original 3.0 behavior remains only when both mode and cleanup are off; F24 extends acknowledgement fencing through native backout. The deletion-manager, activation and complete scenario-6 process checks pass.
...
### F28 — Release gate: Inherited old-client bootstrap limitation
...
This stack does not repair or waive the inherited client limitation. No client binary/profile or test deadline has been changed to hide it. The existing zero setting passed a separate real-broker experiment with each survivor generation, but it is not the migration profile or a mandatory setting chosen by this review.

**Disposition:** retain the failure and require explicit qualification of the unchanged deployed client/tool profile, including cold bootstrap, startup-error handling and approved SLOs. PR 600 makes that a fail-closed live-admission decision; `blocked`, `pending` or a bare `retained` client floor cannot pass. If the unchanged profile is unqualified, stop before the canary. Any client change needs separate owner approval and qualification before the migration baseline. The review can define this gate without selecting or approving a deployed client configuration.
...
### F31 — P1: Unresolved attestations could grant live admission

Preflight accepted any nonempty disposition with an owner and evidence string. A client floor marked `blocked` or merely `retained` could return `kind=live-admission, passed=true`. Truthy non-string fields, whitespace-only runtime qualification and unknown extra decisions could also evade the intended gate.

PR 600 enforces the documented disposition vocabulary, nonblank string fields, explicit unchanged-client qualification and known decision keys. Before tests reproduce successful CLI admission for a blocked client floor; after tests reject it. The empty guide template cannot grant admission. All 93 Python tests and actual RAT pass. The contract-2 JSON shape is unchanged, but formerly permissive values are rejected. No Kafka runtime, client setting, deadline or record assertion changes. These declarations still require real owner review; the validator does not authenticate an approver or prove linked evidence.

### F32 — P2: Fixture reassignment assumes the rest of the cluster is idle

PR 600's process job failed when the reassignment CLI found another active move. The continuous metadata-churn topic legitimately retained a reassignment while its former replica was offline. The CLI rejected the fixture's unrelated move before submitting it, with `Cannot execute because there is an existing partition assignment`. This is neither a broker error nor F28's producer-bootstrap timeout.

PR 601 uses `--additional` for the exact fixture reassignment file. Cancellation needs the same coexistence, but CLI `--throttle` would also rewrite throttle selectors for other active moves. The runner now sets only the cancellation topic's selectors and keeps the existing 1024-byte/s broker rates, then executes without CLI throttle rewriting. Record checks, ISR checks, clients, delays and deadlines are unchanged.

Both call-site regressions fail before and pass after; all 94 Python tests and actual RAT pass. A real-broker probe keeps an unrelated move deliberately pending: the unmodified CLI refuses, the actual new helper succeeds, unrelated assignment/throttle state stays unchanged, and cancelled-topic records verify exactly. The first probe incorrectly required original replica order; Kafka restored `[1,0]`, which the existing scenario already accepts. That failed probe is retained. The corrected scoped probe passes. The separate complete updated process also passes all sixteen client checkpoints, and its independent audit confirms the same client/Connect processes, source fingerprints, archive identities and all 28 required operations.
...
| [601](https://github.com/linkedin/kafka/pull/601) | fixture reassignment coexistence and scoped throttles — verification |
...
|---|---|---|
| Review the named plan and every open public PR | `LI-3.0-TO-3.9-ROLLING-UPGRADE-PLAN.md`; canonical runbook; GitHub inventory | 51 open PRs are listed in both tables. Recheck the exact PR set after any further publication. |
| Explain scope and dependencies | PR responsibility table, heads/bases, stack membership | Separate protocol, handlers, storage metrics, runner, auditor, verifier and release-gate layers. GitHub stack 582 has 40 upgrade PRs; stack 581 has nine. New members were appended and read back. CI 558/559 remain on independent release histories. No release branch was merged. |
| Apply the requested label | GitHub labels | All 49 upgrade PRs have `kafka-upgrade-august-2026`; CI 558/559 do not. |
...
| Use plain, direct English | Plan, review, comment replies and workflow comments | Current wording, repository paths and local links were checked. Historical findings are explicitly separated from current deployment instructions. |
...
| Select symmetric v2/v5/v1 control | Schemas, controller selectors, wire fixtures and retained logs | Both generations have fixtures and complete scenario-6 process coverage on the named runtime archives. F20 makes rotated log checks fail closed too. Approved published-release qualification remains separate. |
...
| Enforce all six phases and unchanged clients | `li_bridge_contract.py`, preflight, persistent client/Streams/Connect and private-API helpers | 94 Python tests pass. F28's exact deployed client-floor qualification is a mandatory release input, not inferred from this review. Native control stays at IBP 3.0 before the separate IBP roll. One 3.0 archive is not the deployed client/tool floor. |
...
| Address every review comment with evidence | Comment ledger, source/test decisions, GraphQL readback | All original 62 replies verified across 51 PRs; no new review threads. Later issue findings have published fixes and explicit qualification limits. Re-fetch after final publication. |
...
- `/tmp/li-review-readback-result.json`: 51 PRs, 62 original threads, no missing/mismatched replies and no new threads at that readback.
...
The 15 previously cancelled intermediate static-check jobs were rerun successfully. Their actual checkout trees match their PR heads, recorded in `/tmp/li-cancelled-static-checks.json`; the cancelled unit/integration jobs are not relabelled as passes. PRs 597 and 598 also pass public process CI. These successful runs do not disprove F28's deterministic client-liveness failure or approve the separate config-zero profile. `/tmp/li-admission-dispositions-regression-before.log` preserves the unqualified-admission failures; `/tmp/li-runtime-attestation-before.log` catches malformed runtime evidence; `/tmp/li-admission-final-tests.log` passes all 93 tests after repair.

Failed runs remain failed. A source, archive or scenario change must be checked against the fingerprint before reuse. The post-f919 documentation/test/live-admission/fixture-orchestration delta is checked separately; it is not a claim that commit identities are identical. Kafka runtime code and the client profile are unchanged. F32's corrected process has its own complete run and independent audit.

`/tmp/li-ci-600-small-evidence` retains the failed CLI refusal and concurrent churn assignment. `/tmp/li-reassignment-coexistence-before.log` records both call-site failures; `/tmp/li-reassignment-coexistence-confirmed` verifies the scoped repair and retains the unrelated pending assignment. `/tmp/li-final-coexisting-process` passes the complete process on immutable f919812ba4 / 8086d17968 archives. `independent-process-audit.json` and `source-and-coverage-audit.json` verify the result: runner a93bed480e, scenario 6, sixteen unchanged-client checkpoints, all 28 required operations and 458 final metadata-churn cycles. This is updated process evidence alongside the earlier full JVM/wrapper bundle, not a fabricated combined verifier fingerprint.
