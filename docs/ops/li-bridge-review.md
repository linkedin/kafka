...
This is not yet a proven resolution of the 60-second CI failure. No client binary/profile or test deadline has been changed to hide it. Qualify the selected client configuration against real brokers and record the deployed floor and owner decision before closing this finding. The existing zero setting is being tested as a separate experiment, not silently substituted into the migration profile. A config opt-out is not automatic approval to change deployed clients.
