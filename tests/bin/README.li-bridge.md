...
The same old clients and Connect worker stay alive throughout. A second old Admin client keeps creating, expanding, cancelling, shrinking and deleting a topic while brokers roll. Fixture reassignments use `--additional` so that unrelated active moves do not trip the CLI's global guard. Cancellation keeps its 1024-byte/s rate but sets only its own topic's throttle selectors; CLI `--throttle` would also rewrite selectors for other active moves. Private APIs run with both old and new jars. The record checks compare contents, not just counts. Ordinary traffic is limited to one record pair every 100 ms so the in-memory test ledger stays bounded; this is not a throughput benchmark.
