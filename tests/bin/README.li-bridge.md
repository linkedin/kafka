...
## Phase and workload evidence (contract version 2, scenario revision 4)
...
Scenario revision 4 keeps a former replica offline through reassignment, deletion and name reuse. It covers assignment to the replacement topic both before and after the broker returns, then promotes the replica and verifies the new bytes. Both broker generations must pass all four combinations. An initial full metadata image may retire unassigned logs; assignment alone cannot identify a recreated topic generation. The cleanup gate reads ZooKeeper topic IDs on the control path and persists identity on new bridge logs. Known obsolete generations may be removed. Unidentified nonempty logs and conflicting current/future copies are retained and require an operator recovery decision. Never force a new ID onto unverified data. Evidence from an older scenario revision does not cover these cases.
