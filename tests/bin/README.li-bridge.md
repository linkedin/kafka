...
## Phase and workload evidence (contract version 2, scenario revision 5)
...
Scenario revision 5 first tests native all-3.0 deletion when the only replica is offline. The online broker must drop the topic from metadata, but ZooKeeper must retain its assignment until the returning replica acknowledges deletion. Recreation must contain only the new records. Keep the cleanup gate enabled after turning bridge mode off; otherwise the old native shortcut can discard the assignment without notifying caches.
