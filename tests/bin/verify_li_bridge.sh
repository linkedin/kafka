...
run clients-bridge-tests run_in_root ./gradlew --no-daemon clients:cleanTest clients:test \
  ...
  --tests 'org.apache.kafka.clients.admin.LiKafkaAdminClientTest' \
  --tests 'org.apache.kafka.clients.consumer.internals.NoOpConsumerRebalanceListenerTest' \
  --tests 'org.apache.kafka.common.utils.PoisonPillProcessTest'
run core-bridge-tests run_in_root ./gradlew --no-daemon core:cleanTest core:test --max-workers=4 \
  ...
  --tests 'kafka.controller.ControllerContextTest.testTopicNameLengthTotalTracksTopicChanges' \
  --tests 'kafka.controller.KafkaControllerTest' \
  --tests 'kafka.admin.AdminZkClientTest.testInvalidMaintenanceBrokerListHasAdminError' \
  --tests 'kafka.cluster.PartitionTest.testLeaderTransferSelectsLowestInSyncFollower' \
  --tests 'kafka.log.UnifiedLogTest.testTruncateTo' \
  --tests 'kafka.network.RequestChannelWatchdogTest' \
  --tests 'kafka.server.ApiVersionManagerTest' \
  --tests 'kafka.server.ApiVersionsRequestTest' \
  --tests 'kafka.server.KafkaActionsTest' \
  --tests 'kafka.server.KafkaConfigTest.testFromPropsInvalid' \
  --tests 'kafka.server.KafkaConfigTest.testEmptyRequestMetricBuckets' \
  --tests 'kafka.server.KafkaConfigTest.testInvalidMaintenanceBrokerList' \
  --tests 'kafka.server.KafkaConfigTest.testInvalidRequestMetricBuckets' \
  --tests 'kafka.server.KafkaConfigTest.testRequestMetricBucketsMustBeOrderedAndNonNegative' \
  --tests 'kafka.server.KafkaServerTest.testRequestChannelWatchdogIntervalTracksConfiguredTimeout' \
  --tests 'kafka.server.DynamicBrokerConfigTest.testProtocolBridgeFlagDefaultsAndDynamicScope' \
  --tests 'kafka.server.LiControllerOperationsTest' \
  --tests 'kafka.server.LeaderTransferManagerTest' \
  --tests 'kafka.server.LiCreateTopicPolicyTest' \
  ...
  --tests 'kafka.server.ObserverTest' \
  --tests 'kafka.server.ProduceRequestInstrumentationTest' \
  --tests 'kafka.server.RackIdMapperTest' \
  --tests 'kafka.server.RequestQuotaTest' \
  --tests 'kafka.server.SaslApiVersionsRequestTest' \
  --tests 'kafka.zk.KafkaZkClientTest.testPaginatedConfigReadPropagatesClientFailure' \
  --tests 'kafka.zk.KafkaZkClientTest.testSetTopicDeletionFlagCreatesMissingZnode'
